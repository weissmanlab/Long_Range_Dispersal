...
if (length(args)!=5) {
  ...
  print(length(args))
  stop("Five input arguments are required - alpha, initial distance, number of trials , number of timesteps, scale parameter", call.=FALSE)
...
for (trial in 1:num_trials) {
   ...
   filestring <- paste("free_and_contrained_CHOSEN_TIME_jump_sizesalpha", alpha , "distance", initial_distance, "trial", (trial -1), ".txt", sep = "") 
   #print(filestring)
...
for (trial in 1:num_trials) {
   ...
   new_filestring <- paste("trajectory_WEIGHT_alpha", alpha , "distance", initial_distance, "trial", (trial -1), ".txt", sep = "") 
   sink(new_filestring)
   cat(weight)
   sink()
