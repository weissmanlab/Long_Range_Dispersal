...
for (int time =0; time < num_time_steps; time++) { for(int T =0; T < num_cdf_steps; T++) {
  if(exp(double(T)/2.0) > time){CDF_of_DCT[T] += dist_of_coalescent_times[time]*timestep;}
}}
...
char OUTPUTFILE333[50];
  ...
  std::stringstream file_name333;
         file_name333 <<  OUTPUTFILE333 << "alpha_value_"<< alpha << "distance_value_" << 
...
char OUTPUTFILE[50];
  ...
  std::stringstream file_name;
         file_name <<  OUTPUTFILE << "alpha_value_"<< alpha << "distance_value_" << initial_position 
         << "rho_inverse_" << rho_inverse << ".txt" ;
