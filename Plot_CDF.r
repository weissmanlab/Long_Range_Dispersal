...
#improper_norm <- max(CDF_table[,5])/.99
print(improper_norm)
...
fun.1 <- function(x) (1- 1/alpha)*x + log(gamma(1+1/alpha)/(((2*D)^(1/alpha))*pi*rho))  + log(1/(1/alpha - 1))  
...
{
CDF_table[,3] <- log(CDF_table[,3] )
CDF_table[,5] <- log(1 - CDF_table[,5] )
CDF_table[,6] <- log(1 - CDF_table[,6] )
CDF_table[,7] <- log(1 - CDF_table[,7] )
