...
p[[k]] <- ggplot() + geom_point(data=Semianalytic_Data, aes(log_of_distance_rescaled, log_odds_of_identity_rescaled), color = "red") + 
        geom_pointrange(data=Simulation_Data, aes(x = log_of_distance_rescaled, y =log_odds_of_identity_rescaled, ymin =V6, ymax =V7), color="blue", pch = 0)  + ggtitle(paste("alpha", alpha))
#print(p[[k]])
k <- k + 1
