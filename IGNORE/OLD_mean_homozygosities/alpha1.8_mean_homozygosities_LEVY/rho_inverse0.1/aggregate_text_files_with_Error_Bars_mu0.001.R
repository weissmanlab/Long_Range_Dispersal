...
mu <- .001  # set whatever mu value you want

super_dummy = 0
sink("outfile_mu0.001")
for (i in 1:nrow(combined_df)){

if( combined_df[i, 2] == mu)
{   dummy <- paste0(log(combined_df[i, 1]), " ", log(combined_df[i, 3]), " ", log(combined_df[i, 3]) - (combined_df[i, 3] - combined_df[i, 4])/combined_df[i, 3], " ", log(combined_df[i, 3]) + (combined_df[i, 5] - combined_df[i, 3] )/combined_df[i, 3])
