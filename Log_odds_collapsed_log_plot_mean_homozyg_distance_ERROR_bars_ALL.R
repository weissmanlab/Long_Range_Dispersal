...
# eliminate zero distance from simulation data for comparison with numerics on logit scale.  Numeric data is already on log scale so we dont need to do this for numerics
...
Simulation_Data <- subset(Simulation_Data, V3 < 1) #mu less than 1

Semianalytic_Data <- subset(Semianalytic_Data, V3 < 1)#mu less than 1
