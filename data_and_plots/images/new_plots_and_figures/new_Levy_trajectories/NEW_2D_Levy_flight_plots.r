...
low_alpha_value <- 0.95
middle_alpha_value <- 5
...
if(alpha > 2){
  set.seed(seed = 102)
  dummy <- data.frame(rmvt(num_gens, sigma=diag(2)*scale_parameter, df=alpha))
}
...
if(QQ == 2)
{x_coord_array_single_timestep[,2] <- x_coord_array_2[1/mu]  -  constant_shift_x -10 #-5 # - 7
  y_coord_array_single_timestep[,2] <- y_coord_array_2[1/mu]  -  constant_shift_y - 25
   x_coord_array_first_timestep[,2] <- x_coord_array_2[1]  -  constant_shift_x -10
  y_coord_array_first_timestep[,2] <- y_coord_array_2[1]  -  constant_shift_y - 25
   x_coord_array_last_timestep[,2] <- x_coord_array_2[tail_cutoff]  -  constant_shift_x  -10
  y_coord_array_last_timestep[,2] <- y_coord_array_2[tail_cutoff]  -  constant_shift_y - 25
  
  
for( TIME in 1:tail_cutoff)
{x_coord_array_2[TIME] <- x_coord_array_2[TIME]  -  constant_shift_x  -10 #-5  #-7 #- 75
	y_coord_array_2[TIME] <- y_coord_array_2[TIME]  -  constant_shift_y - 25
	 }
}
