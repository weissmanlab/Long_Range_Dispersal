...
for (int T = 0; T < num_cdf_steps; T++) {
  List_of_single_trial_CDF[T] = new double[num_trials];
}
for (int T = 0; T < num_cdf_steps; T++) {
  for(int i =0; i < num_trials; i++)
{List_of_single_trial_CDF[T][i] = 0; }
...
for(int trial =0; trial < num_trials; trial++)
 {  Contribution_from_each_trial[trial] = 0;
     ...
     for( int T = 0; T < num_cdf_steps; T++)
     {
      CDF_of_DCT_INDIVIDUAL_TRIAL[T] =  0;
...
{ for(int sample1 = 0; sample1 < num_samples_bootstrapped_cdfs; sample1++)
    ...
    {    
         ...
         tmp2 = Sorted_List_of_bootstrapped_cdfs[T][sample2];
                  if( tmp2 < tmp1)
             {     Sorted_List_of_bootstrapped_cdfs[T][sample1] = tmp2;
...
char OUTPUTFILE333[50];
  sprintf(OUTPUTFILE333, "dist_of_coalescence_times_ROW_FORMAT");
  std::stringstream file_name333;
         file_name333 <<  OUTPUTFILE333 << "alpha_value_"<< alpha << "distance_value_" << initial_position << "rho_inverse_" << rho_inverse << ".txt" ;
         std::string stringfile333;
         file_name333 >> stringfile333; 
  ofstream fout333;
...
char OUTPUTFILE[50];
  sprintf(OUTPUTFILE, "dist_of_coalescence_times_");
...
fout5  << alpha <<  " " <<  rho_inverse  << " " << pow(10, mu)*mu_step << " " << initial_position << " " << mean_homozygosity[mu] << " " << lower_CI[mu] <<  " " << upper_CI[mu] << endl;
...
char OUTPUTFILE22[50];
  ...
  std::stringstream file_name999;
         file_name999 <<  OUTPUTFILE22 << "alpha_value_"<< alpha << "distance_value_" << setw(7) << setfill('0') << initial_position  << "rho_inverse_" << rho_inverse << ".txt" ;
