...
if (length(args)!=5) {
  ...
  print(length(args))
  stop("Five input arguments are required - alpha, number of timesteps, scale parameter, mutation rate, and rho inverse", call.=FALSE)
...
#setwd(dir)
num_time_steps <- as.integer(args[2]) # number of times considered
scale_parameter <- as.double(args[3])
MU <- as.double(args[4])
rho_inverse <- as.double(args[5])
rho <- 1/rho_inverse
